...
class TranslationService:
    ...
    def _translate_text(
        self, text: str, target_lang: str, source_lang: str = LanguageEnum.GERMAN
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            target_lang = (
                target_lang.split("-")[0]
                if target_lang != LanguageEnum.ENGLISH_US.value
                else target_lang
            )
            ...
            logger.info(
                f"Translated {source_lang}: {text} • to {target_lang}: {result.text}"
            )
            return result.text
    ...
    def _get_translation_class(self, obj: T) -> Type[TransT]:
        ...
        raise ValueError(
            f"No translations relationship found in {obj.__class__.__name__}"
        )
            ...
            if isinstance(column.foreign_keys, set) and len(column.foreign_keys) > 0:
                for fk in column.foreign_keys:
                    if not fk.column.table.name.startswith("language"):
                        return column.name
        raise ValueError(
            f"No foreign key column found in translation table {translation_table.__name__}"
        )
    ...
    def _get_translatable_columns(self, translation_table: Type[TransT]) -> List[str]:
        ...
        for column in inspect(translation_table).columns:
            if (
                ...
                and column.name != "language"
                and not column.primary_key
            ):
                columns.append(column.name)
        return columns

    def _get_source_translation(
        self, obj: T, existing_translations: List[TransT]
    ) -> tuple[TransT, LanguageEnum]:
        """
        ...
        eng_trans = next(
            (
                ...
                if t.language == LanguageEnum.ENGLISH_US.value
            ),
            None,
        )
        if eng_trans:
        ...
        de_trans = next(
            (
                t
                for t in existing_translations
                if t.language == LanguageEnum.GERMAN.value
            ),
            None,
        )
        if de_trans:
        ...
        if existing_translations:
            ...
            source_lang = next(
                lang for lang in LanguageEnum if lang.value == first_trans.language
            )
            return first_trans, source_lang

        logger.error(
            f"No existing translations found for table {obj.__tablename__} object {obj.id}"
        )
        raise ValueError(
            f"No existing translations found for table {obj.__tablename__} object {obj.id}"
        )
    ...
    def create_missing_translations(
        ...
        try:
            ...
            existing_languages: Set[str] = {
                trans.language for trans in existing_translations
            }
            ...
            if not missing_languages:
                logger.info(
                    f"Table {obj.__tablename__} object {obj.id} already has all required translations"
                )
                return existing_translations

            # Get source translation and translatable columns
            source_translation, source_language = self._get_source_translation(
                obj, existing_translations
            )
            ...
            for target_lang in missing_languages:
                ...
                logger.info(
                    f"Created translation for table {obj.__tablename__} object {obj.id} to {target_lang.value}"
                )
            ...
            logger.error(
                f"Failed to create translations for table {obj.__tablename__} object {obj.id}: {str(e)}"
            )
            raise
    ...
    def get_translation(self, obj: T, language: LanguageEnum) -> Optional[TransT]:
        ...
        try:
            return next(
                (t for t in obj.translations if t.language == language.value), None
            )
        except Exception as e:
            logger.error(
                f"Failed to get {language.value} translation for {obj.__class__.__name__} {obj.id}: {str(e)}"
            )
            return None
