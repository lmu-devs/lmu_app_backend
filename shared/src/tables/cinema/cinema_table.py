...
class CinemaTable(Base):
    ...
    translations = relationship(
        "CinemaTranslationTable", back_populates="cinema", cascade="all, delete-orphan"
    )
    images = relationship(
        "CinemaImageTable", back_populates="cinema", cascade="all, delete-orphan"
    )
...
class CinemaImageTable(ImageTable, Base):
    ...
    cinema_id = Column(
        String, ForeignKey("cinemas.id", ondelete="CASCADE"), nullable=False
    )
...
class CinemaTranslationTable(LanguageTable):
    ...
    cinema_id = Column(
        String, ForeignKey("cinemas.id", ondelete="CASCADE"), primary_key=True
    )
    title = Column(String)
