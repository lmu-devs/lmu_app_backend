...
def create_error_response(
    error_code: str, message: str, extra: Dict[str, Any] = None
) -> Dict:
...
def handle_error(exc: Exception, context: str = "API") -> Dict:
    if isinstance(exc, (BaseException, APIException)):
        return create_error_response(
            error_code=exc.error_code, message=exc.detail, extra=exc.extra
        )
...
async def api_error_handler(
    request: Any, exc: Union[Exception, APIException]
) -> JSONResponse:
