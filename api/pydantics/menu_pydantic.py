...
def menu_day_to_pydantic(menu_day: MenuDayTable, language: Language, user_id: uuid.UUID = None) -> MenuDay:
    dishes_dto = [
        dish_to_pydantic(
            association.dish,
            language,
...
def menu_days_to_pydantic(menu_days: List[MenuDayTable], language: Language, user_id: uuid.UUID = None) -> Menus:
    return Menus(root=[
        menu_day_to_pydantic(menu_day, language, user_id) 
