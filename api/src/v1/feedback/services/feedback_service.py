...
class FeedbackService:
    ...
    async def create_feedback(self, user_id: uuid.UUID, feedback_data: dict) -> FeedbackTable:
        try:
            ...
            logger.info(
                f"Created new feedback for user {user_id}, screen: {feedback_data['screen']}, rating: {feedback_data['rating']}, app_version: {feedback_data['app_version']} on {feedback_data['system_version']}"
            )
            return new_feedback
