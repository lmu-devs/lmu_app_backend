...
from ..pydantics.cinema_pydantic import movies_to_pydantic, screenings_to_pydantic
...
async def get_movie(
    ...
    movies = await movie_service.get_movies(language, id)
...
async def get_movie_screenings(
    ...
    movie_service = MovieService(db)
    screenings = await movie_service.get_movie_screenings(language)
