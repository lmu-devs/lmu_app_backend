...
@router.get(
    ...
    description="Get all dishes or a specific dish by ID. Authenticated users can also get liked dishes.",
)
async def get_dish(
    id: Annotated[
        UUID,
    ...
    food_logger.info(
        f"Fetched dishes {id} with user_id: {user_id} and only_liked_dishes: {only_liked_dishes} with language: {language}"
    )
...
@router.get(
    "/dishes/dates",
    response_model=DishDates,
    description="Get all canteens where a dish is available in past and future dates",
)
async def read_dish_dates(
    ...
    food_logger.info(
        f"Fetched dish dates {dish_id} with user_id: {current_user.id if current_user else None} and user_likes_canteen: {user_likes_canteen}"
    )
...
@router.post(
    "/dishes/toggle-like",
    response_model=bool,
    description="Authenticated user can toggle like for a dish. Returns True if the dish was liked, False if it was unliked.",
)
