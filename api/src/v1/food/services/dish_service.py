    ...
    async def get_dishes(
        ...
        try:
            ...
            stmt = apply_translation_query(
                base_query=stmt,
                model=DishTable,
                translation_model=DishTranslationTable,
                language=language,
            )
            ...
            if user_id and only_liked:
                stmt = stmt.join(DishLikeTable).where(DishLikeTable.user_id == user_id)
            elif user_id:
                stmt = stmt.outerjoin(
                    DishLikeTable,
                    and_(
            ...
            logger.info(
                f"Found {len(dishes)} dishes matching criteria. dish_id: {dish_id}, user_id: {user_id}, only_liked: {only_liked}"
            )
            return dishes
    ...
    async def get_dates(self, dish_id: int):
        ...
        try:
            stmt = (
                ...
                .join(CanteenTable, MenuDayTable.canteen_id == CanteenTable.id)
                .options(
