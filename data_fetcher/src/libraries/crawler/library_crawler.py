...
import json
# Adjust this import based on where your models.py file is relative to this crawler
# If models.py is in the parent directory (libraries/):
from libraries.models.library_model import Library, ContactInfo, Address, OpeningHours, DaySchedule, CityEnum
# If models.py is at the project root:
import sys
...
class MunichLibraryCrawler:
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            'User-Agent': 'MunichLibraryFetcher/1.0 (https://github.com/your_repo; contact@example.com)' # Replace with actual info
        })
        self.geolocator = Nominatim(user_agent="MunichLibraryCrawler/1.0 (contact@example.com)") # Use specific app name & contact
    ...
    def _parse_address(self, address_text: str) -> Address:
        ...
        if not address_text: return Address()
        ...
        duplicate_indicators = [
            ...
            (r'(?:\b\w+straße|\bstr\.|\bplatz|\ballee)\b.*?(?:\b\w+straße|\bstr\.|\bplatz|\ballee)\b', True),  # Two streets
            (r'\b\d+\s*,\s*\d{5}.*?\b\d+\s*,\s*\d{5}', True),  # Two "number, postal code" patterns
        ]
        ...
        remove_patterns_for_geo = [
            ...
            r'\bRückgebäude\b', r'\bVordergebäude\b', r'\bRgb\.?\b', r'\bVgb\.?\b',
        ...
        room_code_after_city_pattern = r'(\d{5}\s+[\w\s\-äöüÄÖÜß]+?)(\s+[\w\d\.\-]+\s*\d+\s*|\s+\w\d\s+\d+\s*|\s+\d+[a-zA-Z]\.\d+\s*)$'
        ...
        if match:
            ...
            if pc_city_match:
                ...
                street_num_match = re.search(r'^(.*?[\w\.-])\s+(\d+[a-zA-Z]?\s?(?:[-–/]\s?\d+[a-zA-Z]?)?)$', potential_street_part)
                if street_num_match:
        ...
        if city_str:
            ...
            if city_enum is None:
                logger.warning(f"Extracted city '{city_str}' not found in CityEnum. Original address: {address_text_single_line}")
        else:
    ...
    def _parse_opening_hours(self, opening_hours_div: Tag) -> Optional[OpeningHours]:
        ...
        if not opening_hours_div: return None
        ...
        def parse_day_schedule(line: str) -> Optional[DaySchedule]:
            ...
            if 'geschlossen' in line.lower():
                ...
                days_str = days_str_match.group(1).strip() if days_str_match else line # Assume whole line if no explicit "geschlossen"
                ...
                if not time_ranges: return None # No valid time found
            ...
            if not days_str: return None # Could not determine days
            ...
            if not days_list:
                individual_days = re.findall(r'\b(Mo|Di|Mi|Do|Fr|Sa|So|Montag|Dienstag|Mittwoch|Donnerstag|Freitag|Samstag|Sonntag)\b', days_str, re.IGNORECASE)
                day_abbr_map = {'mo': 'Montag', 'di': 'Dienstag', 'mi': 'Mittwoch', 'do': 'Donnerstag', 'fr': 'Freitag', 'sa': 'Samstag', 'so': 'Sonntag'}
            ...
            if not days_list: return None # Failed to parse days
        ...
        for element in opening_hours_div.children:
            ...
            if schedule:
                 ...
                 if not ('semester' in text_lower or 'vorlesungszeit' in text_lower or 'vorlesungsfreie zeit' in text_lower or 'semesterferien' in text_lower):
        ...
        # Cleanup empty lists
        if not opening_hours.semester: opening_hours.semester = None
        if not opening_hours.semester_break: opening_hours.semester_break = None
        ...
        if opening_hours.notes and not opening_hours.semester and not opening_hours.semester_break:
             ...
             if opening_hours.notes.lower() == "geschlossen":
                 opening_hours.semester = [DaySchedule(days=['Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag', 'Sonntag'], times=[{"start": "geschlossen", "end": "geschlossen"}])]
    ...
    def _clean_contact_info(self, contact_data: Dict[str, Any]) -> Optional[ContactInfo]:
        ...
        if contact_data.get("email"):
             potential_emails = contact_data["email"] if isinstance(contact_data["email"], list) else [contact_data["email"]]
        ...
        if unique_phones:
             ...
             cleaned.phone = {"main": unique_phones[0], "additional": unique_phones[1:] if len(unique_phones) > 1 else None}
        ...
        if not any(getattr(cleaned, field) for field in ContactInfo.model_fields if field != 'address' or (cleaned.address and any(getattr(cleaned.address, afield) for afield in Address.model_fields))):
            return None
    ...
    def _parse_library_details(self, url: str, name: str, location_number: List[str]) -> Library:
        ...
        if contact_div:
            ...
            if address_elem: details["contact"]['address'] = address_elem.get_text(separator='\n').strip()
            # Phone
            phone_elem = contact_div.find('p', {'class': 'telefon'})
            if phone_elem: phone_text = phone_elem.get_text(strip=True); details["contact"]['phone'] = re.sub(r'^Telefon:\s*', '', phone_text, flags=re.IGNORECASE).strip()
            # Fax
            fax_elem = contact_div.find('p', {'class': 'fax'})
            if fax_elem: fax_text = fax_elem.get_text(strip=True); details["contact"]['fax'] = re.sub(r'^Fax:\s*', '', fax_text, flags=re.IGNORECASE).strip()
            # Website
            website_div = contact_div.find('div', class_='webadresse')
            if website_div:
                 website_link = website_div.find('a')
                 if website_link and website_link.has_attr('href'): details["contact"]['website'] = urljoin(self.BASE_URL, website_link['href'])
            ...
            if email_link: details["contact"]['email'] = email_link['href'].replace('mailto:', '')
            else: emails_in_contact = self._extract_emails(contact_div.get_text()); details["contact"]['email'] = emails_in_contact
        ...
        if opening_hours_div:
            details["opening_hours"] = self._parse_opening_hours(opening_hours_div)
             ...
             if hours_header:
                  ...
                  hours_text = "\n".join(elem.get_text(strip=True) if isinstance(elem, Tag) else elem for elem in hours_content_elements)
        ...
        transport_header = content_div.find(['h2', 'h3'], string=re.compile(r'Verkehrsanbindung|Anfahrt', re.IGNORECASE))
        if transport_header:
            ...
            for row in table.find_all('tr'):
                ...
                if len(cells) >= 2: # Need at least name and number columns
                    ...
                    if link and link.has_attr('href'):
                        ...
                        location_numbers = [num.strip() for num in number_cell.get_text(separator=',').split(',') if num.strip().isdigit()]
    ...
    def get_libraries(self) -> List[Library]:
        ...
        for i, lib_info in enumerate(base_library_list):
            ...
            try:
                 ...
                 logger.error(f"Unexpected error parsing details for {lib_info['name']} ({lib_info['url']}): {e}", exc_info=True)
