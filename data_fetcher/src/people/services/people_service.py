...
class PeopleService:
    ...
    def __init__(self):
        ...
        self.PEOPLE_QUERIES_FILE = "people_queries.graphql"
    ...
    def _create_new_person(self, person_id: str, person_data: dict):
        ...
        # Handle faculty relation by extracting the faculty id
        faculty_enum = person_data.get("faculty_enum")
        if faculty_enum:
            faculty_id = self.get_enum_value(faculty_enum)
            if faculty_id is not None:
                person_data_clean["faculty"] = {"id": faculty_id}
                self.logger.debug(f"Using faculty id: {faculty_id} for {person_data.get('name', 'UNKNOWN')}")
            else:
        ...
        try:
            # Use mutations.graphql file with CreatePerson operation name
            mutations_file = self.graphql_path / self.MUTATIONS_FILE
            with open(mutations_file, 'r') as f:
                mutations_content = f.read()
            
            # Execute with operation name to specify which mutation to run
            response = self.directus._client.post("/graphql", json={
                "query": mutations_content,
                "variables": person_variables,
                "operationName": "CreatePerson"
            })
            response.raise_for_status()
            response_data = response.json()
            self.logger.info(f"CMS CreatePerson response: {response_data}")
        except Exception as e:
        ...
        try:
            ...
            if person_uuid:
                ...
                # Use mutations.graphql file with CreatePersonDetails operation name
                mutations_file = self.graphql_path / self.MUTATIONS_FILE
                with open(mutations_file, 'r') as f:
                    mutations_content = f.read()
                ...
                response = self.directus._client.post("/graphql", json={
                    ...
                    "operationName": "CreatePersonDetails"
                })
                response.raise_for_status()
                response_data = response.json()
                self.logger.info(f"CMS details response: {response_data}")
            else:
    ...
    def _update_existing_person(self, person_id: str, person_data: dict):
        ...
        # Handle faculty relation
        faculty_enum = person_data.get("faculty_enum")
        if faculty_enum:
            faculty_id = self.get_enum_value(faculty_enum)
            if faculty_id is not None:
                person_updates["faculty"] = {"id": faculty_id}
        ...
        if person_uuid:
            ...
            try:
                # Use mutations.graphql file with UpdatePerson operation name
                mutations_file = self.graphql_path / self.MUTATIONS_FILE
                with open(mutations_file, 'r') as f:
                    mutations_content = f.read()
        ...
        if details_uuid and details_updates:
            ...
            try:
                # Use mutations.graphql file with UpdatePersonDetails operation name
                mutations_file = self.graphql_path / self.MUTATIONS_FILE
                with open(mutations_file, 'r') as f:
                    mutations_content = f.read()
                ...
                response = self.directus._client.post("/graphql", json={
                    "query": mutations_content,
                    "variables": details_variables,
                    "operationName": "UpdatePersonDetails"
                })
                response.raise_for_status()
            except Exception as e:
    ...
    def _upsert_person_roles(self, person_id: str, person_data: dict):
        ...
        for role in roles:
            ...
            try:
                # Use mutations.graphql file with CreatePersonRole operation name
                mutations_file = self.graphql_path / self.MUTATIONS_FILE
                with open(mutations_file, 'r') as f:
                    mutations_content = f.read()
                ...
                response = self.directus._client.post("/graphql", json={
                    "query": mutations_content,
                    "variables": variables,
                    "operationName": "CreatePersonRole"
                })
                response.raise_for_status()
            except Exception as e:
    ...
    def _get_person_by_person_id(self, person_id: str) -> Optional[Dict]:
        ...
        try:
            # Use people_queries.graphql file with GetPersonByPersonId operation name
            queries_file = self.graphql_path / self.PEOPLE_QUERIES_FILE
            ...
            response = self.directus._client.post("/graphql", json={
                ...
                "variables": variables,
                "operationName": "GetPersonByPersonId"
            })
            response.raise_for_status()
    ...
    def _get_person_details_uuid_by_person_id(self, person_id: str) -> Optional[str]:
        ...
        try:
            ...
            # Use GetPersonDetails query from get_person_details.graphql
            query_path = self.graphql_path / self.GET_PERSON_DETAILS_FILE
            variables = {"person_id": person_uuid}
            
            response = self.directus.execute_query_file(query_path, variables)
    ...
    def test_cms_connection(self) -> Dict:
        ...
        try:
            # Test using get_all_people.graphql (single operation file)
