...
class LSFPersonCrawler:
    ...
    def _extract_person_details(self, doc: html.HtmlElement) -> dict:
        ...
        for row in function_rows:
            ...
            if len(cells) >= 2:
                ...
                if institution_name and role_text:
                    ...
                    # Debug: log institution names to see if faculty info is there
                    inst_name = self._clean_text(institution_name[0])
                    logger.debug(f"[Crawler] Found institution: '{inst_name}'")
                    if "Fakultät" in inst_name:
                        logger.debug(f"[Crawler] Institution contains 'Fakultät': '{inst_name}'")

        # Faculty extraction with improved fallback strategies
        faculty_entry = doc.xpath('//div[contains(@style, "padding-left")]//a[contains(text(), "Fakultät")]')
        logger.debug(f"[Crawler] Faculty entry matches (primary): {len(faculty_entry)}")
        
        if not faculty_entry:
            # Try alternative XPath patterns
            faculty_entry = doc.xpath('//a[contains(text(), "Fakultät")]')
            logger.debug(f"[Crawler] Faculty entry matches (alternative): {len(faculty_entry)}")
        
        if not faculty_entry:
            # Try looking for faculty in table cells
            faculty_entry = doc.xpath('//td[contains(text(), "Fakultät")]//a')
            logger.debug(f"[Crawler] Faculty entry matches (table cells): {len(faculty_entry)}")
        
        if not faculty_entry:
            # Try looking for faculty in any text content
            faculty_entry = doc.xpath('//*[contains(text(), "Fakultät")]')
            logger.debug(f"[Crawler] Faculty entry matches (any element): {len(faculty_entry)}")
        
        if faculty_entry:
            # Get the first faculty entry found
            faculty_text = self._clean_text(faculty_entry[0].text_content())
            # Extract just the faculty name if it contains additional text
            if "Fakultät" in faculty_text:
                # Find the faculty name in the text
                import re
                faculty_match = re.search(r'([^,\n]*Fakultät[^,\n]*)', faculty_text)
                if faculty_match:
                    details["faculty"] = faculty_match.group(1).strip()
                else:
                    details["faculty"] = faculty_text
            else:
                details["faculty"] = faculty_text
            logger.debug(f"[Crawler] Faculty extracted: '{details['faculty']}'")
        else:
            # Fallback: try to extract from roles/institutions
            ...
            # Last resort: try to find any faculty reference in the page
            if not details["faculty"]:
                page_text = doc.text_content()
                import re
                faculty_matches = re.findall(r'([^,\n]*Fakultät[^,\n]*)', page_text)
                if faculty_matches:
                    # Take the first faculty match that looks like a complete faculty name
                    for match in faculty_matches:
                        clean_match = self._clean_text(match)
                        if clean_match and len(clean_match) > 5:  # Ensure it's not just "Fakultät"
                            details["faculty"] = clean_match
                            logger.debug(f"[Crawler] Faculty found in page text: '{details['faculty']}'")
                            break

        if details["faculty"]:
            logger.debug(f"[Crawler] Final faculty value: '{details['faculty']}'")
        else:
            # Enhanced debugging when faculty cannot be determined
            logger.warning("[Crawler] Faculty could not be determined for this person")
            
            # Log some debugging information to help understand the page structure
            page_text = doc.text_content()
            faculty_mentions = page_text.count("Fakultät")
            logger.debug(f"[Crawler] Page contains {faculty_mentions} mentions of 'Fakultät'")
            
            if faculty_mentions > 0:
                # Show where "Fakultät" appears in the text
                import re
                faculty_positions = [(m.start(), m.group()) for m in re.finditer(r'[^,\n]*Fakultät[^,\n]*', page_text)]
                logger.debug(f"[Crawler] Faculty mentions found at positions: {faculty_positions[:3]}")  # Show first 3
                
                # Try to extract faculty from the first mention
                if faculty_positions:
                    first_faculty = faculty_positions[0][1]
                    logger.debug(f"[Crawler] First faculty mention: '{first_faculty}'")
                    # Try to use this as a fallback
                    clean_faculty = self._clean_text(first_faculty)
                    if clean_faculty and len(clean_faculty) > 5:
                        details["faculty"] = clean_faculty
                        logger.debug(f"[Crawler] Using fallback faculty: '{details['faculty']}'")
        ...
        logger.debug(f"[Crawler] Summary: roles={len(details['roles'])}, faculty='{details.get('faculty', 'NOT_FOUND')}', courses={len(details['courses'])}")
        if details.get('roles'):
            for i, role in enumerate(details['roles']):
                institutions = role.get('institutions', [])
                logger.debug(f"[Crawler] Role {i+1}: {role.get('lsf_role_enum')} at {len(institutions)} institutions")
                for j, inst in enumerate(institutions):
                    logger.debug(f"[Crawler]   Institution {j+1}: '{inst.get('name', 'NO_NAME')}'")
