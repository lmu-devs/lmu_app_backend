...
class MVVDepartureFetcher:
    ...
    def get_departures(self, stop_id: str) -> dict:
        ...
        try:
            response = self.session.get(
                f"{self.BASE_URL}{self.DEPARTURES_ENDPOINT}",
                params=params
            )
    ...
    def get_available_lines(self, stop_id: str) -> dict:
        ...
        try:
            response = self.session.get(
                f"{self.BASE_URL}{self.LINES_ENDPOINT}",
                params=params
            )
            response.raise_for_status()
            data = response.json()
...
def parse_args():
    ...
    parser.add_argument(
        'stop_id',
        help="Stop ID (e.g., de:09162:70). Expected format 'de:xxxxx:yyyy'."
    )
    parser.add_argument(
        '--save',
        help='Save the response to a JSON file',
        action='store_true'
    )
    parser.add_argument(
        '--raw',
