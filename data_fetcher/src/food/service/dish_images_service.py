...
from shared.src.services import BlurhashService, FileManagementService, TranslationService
...
class DishImageService:
    ...
    def generate_image_with_transparent_background(self, prompt: str, filename: str):
        image_path = self.image_generation_service.generate_image(prompt, height=512, width=512, steps=12, scales=3.5, seed=12345)
    ...
    def generate_dish_image_table(self, dish_obj: DishTable) -> DishImageTable:
        ...
        image_path = self.generate_image_with_transparent_background(f"{self.prompt_prefix} {dish_translation_title}", f"{file_name}-{dish_obj.id}.png")
...
if __name__ == "__main__":
    def main():
        ...
        # result = await service.generate_image_with_transparent_background(
        #     "Simplified 3D Vanille pudding with strawberry sauce", 
        #     "vanille_pudding_with_strawberry_sauce.png"
        # )
        # print(result)
        dish_obj = DishTable(
            id="1",
