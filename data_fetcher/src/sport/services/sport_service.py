...
def add_sport_to_database(db: Session) -> None:
    ...
    try:
        ...
        # Add new data
        _add_sport_courses(db, sport_courses)
        
        db.commit()
        logger.info("Successfully updated sport courses in database")
        
    except Exception as e:
        db.rollback()
        logger.error(f"Failed to update sport courses: {str(e)}")
        raise
...
def _clear_existing_data(db: Session) -> None:
    try:
        ...
        db.commit()
        logger.info("Successfully cleared existing sport data")
    except Exception as e:
        db.rollback()
        logger.error(f"Error clearing existing data: {str(e)}")
        raise

def _add_sport_courses(db: Session, sport_courses: List[SportCourse]) -> None:
    ...
    for sport in sport_courses:
        ...
        for course in sport.courses:
            ...
            if course.id in processed_course_ids:
                logger.warning(f"Skipping duplicate course ID: {course.id}")
                continue
