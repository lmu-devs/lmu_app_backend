...
class AliasGenerationService:
    def __init__(self, llm_factory: LLMFactory | None = None):
        system_message = SystemMessage(content="You are a helpful assistant that generates aliases for a given input. Try to be creative and think of FITTING, SIMPLE and SINGE WORD search terms that a user would use to find the given input. DONT REPEAT SIMILAR ALIASES. SINGLE WORDS ONLY. DONT USE CAMEL CASE.")
