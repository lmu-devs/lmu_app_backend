...
class LinkBenefitService:
    ...
    def _add_missing_aliases(self):
        ...
        benefit_translations = self.db.query(LinkBenefitTranslationTable).filter(LinkBenefitTranslationTable.aliases.is_(None)).all()
        
        for benefit_translation in benefit_translations:
            aliases = self.alias_generation_service.generate_alias(benefit_translation.title, benefit_translation.description)
